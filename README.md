...
### 项目讲述主线（建议2-3分钟）
1. **背景切入**：从"土木水利转供应链"的跨学科视角出发，说明选择库存项目作为切入点的原因——库存是供应链的核心节点，且能充分发挥数据分析能力
2. **问题发现**：通过EDA发现数据的间歇性需求特征，指出经典模型的局限
3. **方案设计**：ABC分类 + 差异化安全库存 + EOQ + 模拟验证的完整逻辑链
4. **结果呈现**：用服务水平93.79%、周转率1.75等量化指标收尾
5. **技术亮点**：Streamlit Dashboard和RAG Agent体现技术落地能力

### 建议准备的可视化材料
- 整体架构图（本README中的Mermaid图）
- ABC分类帕累托图
- 补货模拟前后对比图
- Dashboard截图（2-3张关键页面）
- RAG Agent交互演示视频（1分钟内）
